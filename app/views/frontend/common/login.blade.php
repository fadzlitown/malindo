		...
		<div class="col-xs-12 col-sm-8 col-md-6 col-sm-offset-2 col-md-offset-3">
			<div class="row">
				...
				<div class="input-group {{$var = $errors->first('email')}} {{ ($var !== '') ? 'has-error' : ''}}">
					...
					<?php echo Form::text('email', 'rifkiyandhi@gmail.com', array('class' => 'form-control', 'placeholder' => 'email address', 'id' => 'email')); ?>
				</div>
				<span class="help-block">{{$errors->first('email')}}</span>

				<div class="input-group {{$var = $errors->first('password')}} {{ ($var !== '') ? 'has-error' : ''}}">
					<span class="input-group-addon"><i class="fa fa-lock"></i></span>
					<?php echo Form::password('password', array('class' => 'form-control', 'placeholder' => 'password', 'id' => 'password', 'value' => 'password')); ?>
				</div>
